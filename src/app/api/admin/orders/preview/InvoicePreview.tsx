...
export function InvoicePreview({ data }: { data: Preview }) {
  const { cliente: C, ordine: O, totali: T } = data;
    ...
    <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <div className="mt-1 text-xs text-neutral-400">Documento fiscale (bozza)</div>
        </div>
        <div className="text-right">
          <div className="text-lg font-semibold">SOS24ORE.it</div>
          <div className="text-xs text-neutral-400">P.IVA / CF su anagrafica venditore</div>
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <div className="rounded-xl border border-white/10 p-4">
          ...
          <div className="mt-2 space-y-1 text-sm text-neutral-300">
            <div>Metodo: <b>SDD</b></div>
            <div>Rate: {O.rate}</div>
            {C.iban && <div>IBAN: {C.iban}</div>}
            {C.banca && <div>Intestatario: {C.banca}</div>}
          </div>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            <tr className="border-t border-white/10">
              ...
              <td className="p-3">{eur(O.prezzo_unit)}</td>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <div className="rounded-xl border border-white/10 p-4 text-sm">
          ...
          <div className="mt-1 text-neutral-300">
            Rate: {O.rate} • Scadenza fattura: {O.scadenza_giorni} giorni
          </div>
          {C.sottoscrittore && (
        ...
        <div className="rounded-xl border border-white/10 p-4">
          <div className="space-y-1 text-sm">
            <div className="flex justify-between"><span>Imponibile</span><b>{eur(T.imponibile)}</b></div>
            {T.sconto ? (
              <div className="flex justify-between">
                <span>Sconto</span><b>-{eur(T.sconto)}</b>
              </div>
            ) : null}
            <div className="flex justify-between"><span>Imponibile netto</span><b>{eur(T.imponibile_netto)}</b></div>
            <div className="flex justify-between"><span>IVA {O.iva}%</span><b>{eur(T.iva)}</b></div>
            <div className="mt-2 flex justify-between text-base">
              <span>Totale</span><b>{eur(T.totale)}</b>
            </div>
            {O.rate > 1 && (
              <div className="flex justify-between">
                <span>Rata ({O.rate})</span><b>{eur(T.rata)}</b>
              </div>
            )}
          </div>
        </div>
